...
router.get('/booking-upcoming/:worker', async (req, res) => {
    ...
    try {
        ...
        const bookings = user ? await bookingModel.find({ 'client': worker, isPaid: true, completed: false }) : await bookingModel.find({ worker: worker, isPaid: true, completed: false, started: false })
