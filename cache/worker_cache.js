...
module.exports.getWorkerTokenCache = async function getWorkerTokenCache(
  req,
  res,
  next
) {
...
// reviews cache
module.exports.getReviewsCache = async function getReviewsCache(
  req,
  res,
  next
) {
  const reviews = myCache.get(`reviews/${req.params.workerId}`);
...
// comments cache
module.exports.getCommentsCache = async function getCommentsCache(
  req,
  res,
  next
) {
  const comments = myCache.get(`comments/${req.params.postId}`);
...
// booking cache
module.exports.getBookingCache = async function getBookingCache(
  req,
  res,
  next
) {
  const booking = myCache.get(`booking/${req.params.workerId}`);
...
module.exports.getUpcomingBookingCache = async function getUpcomingBookingCache(
  req,
  res,
  next
) {
  const booking = myCache.get(`upcoming-bookings/${req.params.workerId}`);
  console.log("cached upcoming booking ", booking);
  if (booking !== null && booking !== undefined) {
    console.log("Upcoming Booking found in cache");
    return res.status(200).json({
      msg: "Upcoming Booking Found",
      status: 200,
      success: true,
      bookings: JSON.parse(booking),
    });
...
module.exports.getPendingBookingCache = async function getPendingBookingCache(
  req,
  res,
  next
) {
...
module.exports.getInProgressBookingCache =
  async function getInProgressBookingCache(req, res, next) {
...
module.exports.getCompletedBookingCache =
  async function getCompletedBookingCache(req, res, next) {
    const booking = myCache.get(`completed-bookings/${req.params.workerId}`);
    console.log("cached completed booking ", booking);
    if (booking !== null && booking !== undefined) {
      console.log("Completed Booking found in cache");
      return res.status(200).json({
        msg: "Completed Booking Found",
        status: 200,
        success: true,
        bookings: JSON.parse(booking),
      });
...
// get cancelled cache
module.exports.getCancelledBookingCache =
  async function getCancelledBookingCache(req, res, next) {
    const booking = myCache.get(`cancelled-bookings/${req.params.workerId}`);
    console.log("cached cancelled booking ", booking);
    if (booking !== null && booking !== undefined) {
      console.log("Cancelled Booking found in cache");
      return res.status(200).json({
        msg: "Cancelled Booking Found",
        status: 200,
        success: true,
        bookings: JSON.parse(booking),
      });
...
// get worker review
module.exports.getWorkerReviewCache = async function getWorkerReviewCache(
  req,
  res,
  next
) {
  const reviews = myCache.get(`worker-review/${req.params.workerId}`);
...
module.exports.getPopularWorkersCache = async function getPopularWorkersCache(
  req,
  res,
  next
) {
  const workers = myCache.get(`popular-workers`);
  const highestRated = myCache.get(`popular-workers-sorted`);
...
// paid bookings cache
module.exports.getPaidBookingsCache = async function getPaidBookingsCache(
  req,
  res,
  next
) {
